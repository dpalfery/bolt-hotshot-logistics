...
namespace HotshotLogistics.Core.Extensions
{
    ...
    public static class OptionalAzureAppConfigurationExtensions
    {
            ...
            bool useDefaultAzureCredential = true)
        {
            ...
            IConfiguration lookup = builder as IConfiguration ?? builder.Build();

            string? connectionString = lookup.GetConnectionString(ConnectionStringName)
            ...
            string? endpoint = lookup[EndpointKey] ?? lookup["AZURE_APPCONFIGURATION_ENDPOINT"];
