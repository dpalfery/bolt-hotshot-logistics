...
namespace HotshotLogistics.Tests
{
    ...
    public class CustomerControllerTests
    {
        ...
        private static ICustomer CreateTestCustomer(string id, string companyName, bool isActive = true)
        {
            var mockCustomer = new Mock<ICustomer>();
            mockCustomer.Setup(c => c.Id).Returns(id);
            mockCustomer.Setup(c => c.CompanyName).Returns(companyName);
            mockCustomer.Setup(c => c.IsActive).Returns(isActive);
            mockCustomer.Setup(c => c.CreditLimit).Returns(10000m);
            mockCustomer.Setup(c => c.Contacts).Returns(new List<Contact>
            {
                new Contact { Email = $"{id}@test.com", Phone = "555-0123", IsPrimary = true }
            });
            mockCustomer.Setup(c => c.CreatedAt).Returns(DateTime.UtcNow);
