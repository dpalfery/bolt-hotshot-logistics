...
namespace HotshotLogistics.Application.Validators
{
    ...
    public class DriverRegistrationValidator : AbstractValidator<DomainDriverDto>
    {
        ...
        public DriverRegistrationValidator()
        {
            ...
            RuleFor(x => x.PersonalInfo.PhoneNumber)
                ...
                .WithMessage("A valid US phone number is required.")
            ...
            RuleFor(x => x)
                .Must(x => x.License.LicenseExpiryDate > DateTime.UtcNow.AddDays(30))
                .WithMessage("License must be valid for at least 30 days.")
                .Must(x => x.License.LicenseExpiryDate < DateTime.UtcNow.AddYears(10))
                .WithMessage("License expiry date cannot be more than 10 years in the future.");

            // Business rules for driver registration
            RuleFor(x => x)
                .Must(BeAtLeast18YearsOld).WithMessage("Driver must be at least 18 years old.")
                .Must(HaveValidLicenseForRegistration)
                .WithMessage("License must be valid and meet minimum requirements for registration.");
        }
    ...
    public class DriverUpdateValidator : AbstractValidator<DomainDriverDto>
    {
        ...
        public DriverUpdateValidator()
        {
            ...
            RuleFor(x => x.PersonalInfo.FirstName)
                .NotEmpty().When(x => !string.IsNullOrEmpty(x.PersonalInfo.FirstName))
                .WithMessage("First name cannot be empty if provided.")
                .MaximumLength(50).WithMessage("First name cannot exceed 50 characters.")
                .Matches(@"^[a-zA-Z\s\-']+$").When(x => !string.IsNullOrEmpty(x.PersonalInfo.FirstName))
                .WithMessage("First name can only contain letters, spaces, hyphens, and apostrophes.");

            RuleFor(x => x.PersonalInfo.LastName)
                .NotEmpty().When(x => !string.IsNullOrEmpty(x.PersonalInfo.LastName))
                .WithMessage("Last name cannot be empty if provided.")
                .MaximumLength(50).WithMessage("Last name cannot exceed 50 characters.")
                .Matches(@"^[a-zA-Z\s\-']+$").When(x => !string.IsNullOrEmpty(x.PersonalInfo.LastName))
                .WithMessage("Last name can only contain letters, spaces, hyphens, and apostrophes.");

            RuleFor(x => x.PersonalInfo.Email)
                .EmailAddress().When(x => !string.IsNullOrEmpty(x.PersonalInfo.Email))
                .WithMessage("A valid email address is required.")
                .MaximumLength(254).WithMessage("Email address cannot exceed 254 characters.");

            RuleFor(x => x.PersonalInfo.PhoneNumber)
                .Matches(@"^\+?1?[-.\s]?\(?([0-9]{3})\)?[-.\s]?([0-9]{3})[-.\s]?([0-9]{4})$")
                .When(x => !string.IsNullOrEmpty(x.PersonalInfo.PhoneNumber))
                .WithMessage("A valid US phone number is required.")
                .MaximumLength(20).WithMessage("Phone number cannot exceed 20 characters.");

            RuleFor(x => x.License.LicenseNumber)
                .MaximumLength(20).When(x => !string.IsNullOrEmpty(x.License.LicenseNumber))
                .WithMessage("License number cannot exceed 20 characters.")
                .Matches(@"^[A-Z0-9\-]+$").When(x => !string.IsNullOrEmpty(x.License.LicenseNumber))
                .WithMessage("License number can only contain letters, numbers, and hyphens.");

            RuleFor(x => x.License.LicenseExpiryDate)
                .GreaterThan(DateTime.UtcNow).When(x => x.License.LicenseExpiryDate != default)
                .WithMessage("License expiry date must be in the future.")
                .LessThan(DateTime.UtcNow.AddYears(10)).When(x => x.License.LicenseExpiryDate != default)
                .WithMessage("License expiry date cannot be more than 10 years in the future.");
        }
    }
}
