...
namespace HotshotLogistics.Application.Services
{
    ...
    public class MappingServiceFactory : IMappingServiceFactory
    {
        ...
        {
            string providerName = NormalizeProviderName(_configuration["Mapping:Provider"] ?? "Mock");
            Dictionary<string, IMappingService> mappingServiceDict = _serviceProvider.GetServices<IMappingService>()
                .GroupBy(s => s.GetType())
                .Select(g => g.First())
                .ToDictionary(
            ...
            if (mappingServiceDict.TryGetValue(providerName, out IMappingService? service))
            {
            ...
            if (mappingServiceDict.TryGetValue("MockMapping", out IMappingService? mockService) ||
                mappingServiceDict.TryGetValue("Mock", out mockService))
            {
                return mockService;
            }

            throw new InvalidOperationException(
                $"Unsupported mapping provider: {providerName} and no Mock service found.");
        }
