...
namespace HotshotLogistics.Application.Services
{
    ...
    public class ConnectionManagerService : IConnectionManagerService
    {
        ...
        public ConnectionManagerService(IDistributedCache cache, ILogger<ConnectionManagerService> logger,
            ISignalRClientWrapper signalRClient)
        {
            _cache = cache;
            _logger = logger;
        ...
        {
            string key = $"user_connections:{userId}";
            List<string> connections = await GetUserConnectionsListAsync(userId);
            ...
            if (!connections.Contains(connectionId))
            {
                ...
                string connectionKey = $"connection_user:{connectionId}";
        ...
        {
            string key = $"user_connections:{userId}";
            string? connectionsJson = await _cache.GetStringAsync(key);
        ...
        public async Task RemoveConnectionAsync(string connectionId)
        {
            ...
            string? userId = await GetUserIdAsync(connectionId);
        ...
        {
            string key = $"connection_user:{connectionId}";
            return await _cache.GetStringAsync(key);
        }

        public async Task AddToGroupAsync(string connectionId, string groupName)
        {
            string key = $"group_connections:{groupName}";
            List<string> connections = await GetGroupConnectionsListAsync(groupName);
        ...
        public async Task RemoveFromGroupAsync(string connectionId, string groupName)
        {
            ...
            if (connections.Remove(connectionId))
            {
                ...
                _logger.LogInformation("Removed connection {ConnectionId} from group {GroupName}", connectionId,
                    groupName);
            }
        }
        ...
        public async Task SendToUsersAsync(IEnumerable<string> userIds, string methodName, params object[] args)
        {
            try
            {
                ...
                _logger.LogInformation("Sent message {MethodName} to {UserCount} users", methodName, userList.Count);
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> HandleReconnectionAsync(string userId, string connectionId, int retryCount = 0)
        {
            ...
            {
                _logger.LogWarning(
                    "Max reconnection attempts ({MaxRetries}) reached for user {UserId}, connection {ConnectionId}",
            ...
            try
            {
                ...
                List<string> existingConnections = await GetUserConnectionsListAsync(userId);
                if (existingConnections.Contains(connectionId))
                {
                    _logger.LogInformation("Connection {ConnectionId} for user {UserId} is still active", connectionId,
                        userId);
                    return true;
                }
                ...
                _logger.LogInformation(
                    "Successfully reconnected user {UserId} with connection {ConnectionId} on attempt {Attempt}",
            ...
            catch (Exception ex)
            {
                _logger.LogWarning(ex,
                    "Reconnection attempt {Attempt} failed for user {UserId}, connection {ConnectionId}. Retrying...",
                    retryCount + 1, userId, connectionId);

                // Exponential backoff
                double delay = baseDelayMs * Math.Pow(2, retryCount);
        ...
        {
            string key = $"user_groups:{userId}";
            List<string> groups = await GetUserGroupsAsync(userId);
        ...
        {
            string key = $"user_groups:{userId}";
            string? groupsJson = await _cache.GetStringAsync(key);
