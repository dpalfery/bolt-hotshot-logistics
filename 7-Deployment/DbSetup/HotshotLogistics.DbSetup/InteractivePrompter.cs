...
namespace HotshotLogistics.DbSetup
{
    public class InteractivePrompter
    {
            ...
            if (IsNonInteractive())
            {
                if (required && string.IsNullOrEmpty(defaultValue))
                {
                    throw new InvalidOperationException(
                        $"Non-interactive mode: Required input '{message}' not provided and no default value available.");
                }

                _logger.LogInformation("Non-interactive mode: Using default value for '{Message}': {DefaultValue}",
                    message, defaultValue ?? "none");
                return defaultValue;
            }
        ...
        public string PromptForPassword(string message, bool confirm = true)
        {
            ...
            {
                throw new InvalidOperationException(
                    "Non-interactive mode: Cannot prompt for password. Provide --password or set HOTSHOT_DB_PASSWORD environment variable.");
            }
        ...
        public bool PromptForDestructiveAction(string actionDescription, bool force = false)
        {
            ...
            {
                _logger.LogWarning(
                    "Force mode enabled: Skipping confirmation for destructive action: {ActionDescription}",
                    actionDescription);
                return true;
            }

            string message =
                $"WARNING: This action will {actionDescription}. This cannot be undone.\nDo you want to continue? (y/N): ";
        ...
        private static bool IsNonInteractive()
        {
            ...
            return Console.IsInputRedirected || Console.IsOutputRedirected ||
                   Environment.GetCommandLineArgs().Any(arg => arg.Contains("--non-interactive"));
        }
