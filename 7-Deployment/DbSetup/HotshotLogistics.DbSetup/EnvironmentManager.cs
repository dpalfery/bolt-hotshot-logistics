...
namespace HotshotLogistics.DbSetup
{
    public class EnvironmentManager
    {
        ...
        private bool TrySetSystemEnvironmentVariable(string variableName, string value, bool requireAdmin = false)
        {
            ...
            _logger.LogInformation("Attempting to set system environment variable {VariableName} on {Platform}",
            ...
            try
            {
                switch (platform)
                {
                    ...
                    default:
                        _logger.LogError("Unsupported platform for environment variable persistence: {Platform}",
                            platform);
                        return false;
                }
            }
        ...
        public bool PersistWithConsent(string variableName, string value, bool nonInteractive)
        {
            if (nonInteractive)
            {
                _logger.LogInformation(
                    "Non-interactive mode: Environment variable {VariableName} will not be persisted to system. Use process environment or manual setup.",
                    variableName);
                return false;
            }
            ...
            bool success = TrySetSystemEnvironmentVariable(variableName, value, true);
        ...
        private bool TrySetWindowsEnvironmentVariable(string variableName, string value, bool requireAdmin)
        {
            try
            {
                ...
                // Verify it was set
                string? verifyValue = Environment.GetEnvironmentVariable(variableName);
                ...
                if (requireAdmin && IsWindowsAdministrator())
                {
                    ...
                    if (verifyValue == value)
                    {
                        ...
                        Console.WriteLine(
                            "Note: You may need to log out and log back in for all processes to see this variable.");
                        return true;
                    }
                }
        ...
        private bool TrySetUnixEnvironmentVariable(string variableName, string value)
        {
            try
            {
                ...
                string shellProfile = GetShellProfilePath();

                if (!string.IsNullOrEmpty(shellProfile))
                {
                    string exportLine = $"export {variableName}='{value.Replace("'", "'\"'\"'")}'";
                    string fullProfilePath = Path.Combine(homeDir, shellProfile);

                    if (File.Exists(fullProfilePath))
                    {
                        string existingContent = File.ReadAllText(fullProfilePath);
                        if (!existingContent.Contains(exportLine))
                        {
                            ...
                            Console.WriteLine(
                                $"Run 'source ~/{shellProfile}' or restart your shell to load the variable.");
                        }
                    }
                    else
                    {
        ...
        {
            string shell = Environment.GetEnvironmentVariable("SHELL")?.ToLower() ?? "";
        ...
        private void PrintManualInstructions(string variableName, string value)
        {
            ...
            switch (platform)
            {
                case Platform.Windows:
                    ...
                    Console.WriteLine(
                        $"Windows PowerShell (machine): [Environment]::SetEnvironmentVariable('{variableName}', '{value}', 'Machine')");
